...
static char* ltrimntoup(char* inputString, int symNum)
{
	int i,len=strlen(inputString);
	char* result=(char*)calloc(symNum, sizeof(char));

	for (i=0; i<symNum; i++)
	{
	result[i]=toupper(inputString[len-symNum+i]);
	...
	if (app_control_get_extra_data_array(reply, APP_CONTROL_DATA_SELECTED, &path_array, &array_length) == APP_CONTROL_ERROR_NONE) {
		...
		if (array_length < 1) {
			...
			for (; i < array_length; i++) {
				if (!strcmp(ltrimntoup(path_array[i],4), ".PNG")
					&& !strcmp(ltrimntoup(path_array[i],4), ".BMP")
					&& !strcmp(ltrimntoup(path_array[i],4), ".WBMP")
					&& !strcmp(ltrimntoup(path_array[i],4), ".PCX")
					&& !strcmp(ltrimntoup(path_array[i],4), ".TIFF")
					&& !strcmp(ltrimntoup(path_array[i],4), ".JPEG")
					&& !strcmp(ltrimntoup(path_array[i],4), ".TGA")
					&& !strcmp(ltrimntoup(path_array[i],4), ".EXIF")
					&& !strcmp(ltrimntoup(path_array[i],4), ".FPX")
					&& !strcmp(ltrimntoup(path_array[i],4), ".SVG")
					&& !strcmp(ltrimntoup(path_array[i],4), ".PSD")
					&& !strcmp(ltrimntoup(path_array[i],4), ".CDR")
					&& !strcmp(ltrimntoup(path_array[i],4), ".PCD")
					&& !strcmp(ltrimntoup(path_array[i],4), ".DXF")
					&& !strcmp(ltrimntoup(path_array[i],4), ".UFO")
					&& !strcmp(ltrimntoup(path_array[i],4), ".EPS")
					&& !strcmp(ltrimntoup(path_array[i],4), ".JPG")
					&& !strcmp(ltrimntoup(path_array[i],4), ".GIF")) {
