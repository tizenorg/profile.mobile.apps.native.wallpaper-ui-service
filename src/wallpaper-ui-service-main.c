...
static void wallpaper_preview_main();
...
static char* get_file_extension_from_path(char* inputString, int symNum)
{
	...
	if (app_control_get_extra_data_array(reply, APP_CONTROL_DATA_SELECTED, &path_array, &array_length) == APP_CONTROL_ERROR_NONE) {
		...
		if (array_length < 1) {
			...
			for (; i < array_length; i++) {
				...
				file_ext = get_file_extension_from_path(path_array[i],4);
