...
static char* _get_filename_extension_in_uppercase(char *inputString)
{
	WALLPAPERUI_TRACE_BEGIN;
	char *pointChar = strrchr(inputString,'.');
	int startPos = pointChar - inputString;
	int len = strlen(inputString)-startPos;
	int i;
	...
	for (i = 0; i < len; i++)
	{
  	    result[i] = toupper(inputString[startPos+i]);
	}
	WALLPAPERUI_TRACE_END;
	return result;
}
	...
	if (app_control_get_extra_data_array(reply, APP_CONTROL_DATA_SELECTED, &path_array, &array_length) == APP_CONTROL_ERROR_NONE) {
		...
		if (array_length < 1) {
			...
			for (; i < array_length; i++) {
				...
				file_ext = _get_filename_extension_in_uppercase(path_array[i]);
